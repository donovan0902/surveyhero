...
const extractedValueValidator = v.union(
  v.string(),
  v.number(),
  v.boolean(),
  v.null(),
...
export const getOrCreateVoiceResponse = internalMutation({
  ...
  handler: async (
    ...
    return { responseId, respondentId: user._id };
  },
});
...
async function ingestPostCallWebhook(
  ...
  args: {
    ...
    const responseIdFromElevenLabsUserId = args.responseIdFromElevenLabsUserId
      ? ctx.db.normalizeId(
          "surveyResponses",
          args.responseIdFromElevenLabsUserId,
        )
      : null;
    const byElevenLabsUserId = responseIdFromElevenLabsUserId
      ? await ctx.db.get(responseIdFromElevenLabsUserId)
      : null;
    ...
    const questions = await ctx.db
      ...
      .withIndex("by_surveyId_and_order", (q) =>
        q.eq("surveyId", surveyResponse.surveyId),
      )
      .order("asc")
      .collect();
    ...
    for (const result of args.dataCollectionResults) {
      const questionId = questionsByDataCollectionId.get(
      ...
      await ctx.db
        ...
        .withIndex("by_surveyResponseId_and_questionId", (q) =>
          q
            .eq("surveyResponseId", surveyResponse._id)
...
export const syncAgentForSurvey = action({
  ...
  handler: async (
    ...
    const context = await ctx.runQuery(
      internal.elevenlabs.getOwnedSurveyAgentContext,
      { surveyId: args.surveyId },
    );
...
export const startVoiceResponse = action({
  ...
  handler: async (
    ...
    const context = await ctx.runQuery(
      internal.elevenlabs.getPublishedSurveyAgentContext,
      { surveyId: args.surveyId },
    );
    const { agentId } = await syncSurveyAgent(ctx, context);
    const response = await ctx.runMutation(
      internal.elevenlabs.getOrCreateVoiceResponse,
      { surveyId: args.surveyId },
    );
...
async function syncSurveyAgent(
  ...
  if (
    context.survey.elevenLabsAgentId &&
    context.survey.elevenLabsAgentConfigHash === configHash
  ) {
    return {
  ...
  const agentId = await upsertElevenLabsAgent(
    context.survey.elevenLabsAgentId,
    body,
  );
...
function getFollowUpInstruction(
  behavior: Doc<"questions">["followUpBehavior"],
): string {
...
function getDataCollectionType(
...
async function upsertElevenLabsAgent(
  ...
  if (!response.ok) {
    throw new Error(
      `ElevenLabs agent sync failed (${response.status}): ${await response.text()}`,
    );
  }
...
async function getSignedUrl(agentId: string): Promise<string> {
  ...
  if (!response.ok) {
    throw new Error(
      `ElevenLabs signed URL request failed (${response.status}): ${await response.text()}`,
    );
  }
